...
void UserInterfaceClass::setButton(uint16_t xStart, uint16_t yStart, uint16_t xStop, uint16_t yStop, uint16_t clickReturn, bool isRound, String btnText)
{
    ...
    this->align = GUI_Align_Text_Center;
...
bool UserInterfaceClass::getIsPrintable()
{
    bool isPrintable;
    (CHECK_BIT(this->btnSettings, Settings_isPrintable)) ? isPrintable = true : isPrintable = false;
    return isPrintable;
}
...
void UserInterfaceClass::setClickReturn(uint16_t size)
{
    this->clickReturn = size;
}